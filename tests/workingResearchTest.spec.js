...
test('Working Research Test with Correct Selectors', async ({ page }) => {
  console.log("=== WORKING RESEARCH TEST ===");
  ...
  // Fill in research topic using the correct placeholder
  ...
  // Wait a bit more to see if report is generated
  console.log("Waiting a bit more for report generation...");
  ...
  await page.screenshot({ path: 'working-research-test-result.png', fullPage: true });
  console.log("Screenshot saved as working-research-test-result.png");
  ...
  if (reportContent > 0 || markdownElements > 0 || paragraphElements > 5) {
    ...
    console.log("⚠️ Report generation status unclear - still in progress or encountered issues");
  }
});